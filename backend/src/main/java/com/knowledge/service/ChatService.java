    ...
    private String buildSystemPrompt(boolean webSearchEnabled, boolean hasKnowledge) {
        if (webSearchEnabled && hasKnowledge) {
            return """
                    ...
                    回答规则：
                    1. 「已有相关知识」是你积累的个人知识，仔细阅读并融合多条相关条目内容，形成完整、连贯的回答
                    2. 「搜索结果」提供最新的外部信息，用于补充已有知识的不足或获取最新动态
                    3. 综合多条知识时，提炼共性和差异，组织成结构化的回答（如"核心概念-细节补充-对比分析"）
                    4. 当两者信息冲突时，明确说明差异，并以搜索结果为准（更新、更权威）
                    5. 如果已有知识仅部分相关，明确指出回答了哪些方面、哪些方面知识库尚未覆盖
                    6. 信息不足时请明确说明「未找到足够信息」，并建议用户可以补充相关知识
                    7. 回答应清晰、结构化，使用 Markdown 格式
                    8. 引用来源时使用 [1]、[2] 编号，对应末尾参考来源列表
                    9. 末尾添加「---\n参考来源：」章节，格式：\n[1] [来源标题](完整链接)
                    10. 用中文回答
                    """;
            ...
            return """
                    ...
                    回答规则：
                    1. 搜索结果优先于你自身的参数知识
                    2. 综合多条搜索结果，提炼关键信息，形成结构化回答
                    3. 信息不足时请明确说明「未找到足够信息」
                    4. 回答应清晰、结构化，使用 Markdown 格式
                    5. 引用来源时使用 [1]、[2] 编号，对应末尾参考来源列表
                    6. 末尾添加「---\n参考来源：」章节，格式：\n[1] [来源标题](完整链接)
                    7. 用中文回答
                    """;
            ...
            return """
                    ...
                    回答规则：
                    1. 仔细阅读「已有相关知识」中的多条历史问答，融合相关内容形成完整回答
                    2. 如果多条知识之间有联系，组织成结构化回答（如"核心概念-细节补充-实际示例"）
                    3. 如果已有知识仅部分相关，明确指出回答了哪些方面、哪些方面知识库尚未覆盖
                    4. 如果已有知识不足，可以结合你自身的通用知识回答，但需标注"此部分来自通用知识"
                    5. 信息不足时请明确说明「未找到足够信息」，并建议用户可以补充相关知识
                    6. 回答应清晰、结构化，使用 Markdown 格式
                    7. 用中文回答
                    """;
        } else {
            return """
